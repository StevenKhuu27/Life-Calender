...
scale = 2
image_size = [int(1440*scale), int(3216*scale)]
radius = 20*scale
num_dots_x = 15
num_dots_y = 25
days = 365
...
week_y = margin_y - 120  # above first row
week_radius = 20 *scale
week_gap = 90
start_x = new_image.width // 2 - (3 * week_gap)
...
# Week counter
for i in range(7):
    x = start_x + i * week_gap
    y = week_y

    if i < weekday_index:
        colour = ((40, 120, 110))
    elif i == weekday_index:
        colour = (0, 204, 184)
    else:
        colour = dark_grey_colour

    draw.circle((x, y), week_radius, fill=colour)
...
wrapped_lines = textwrap.wrap(quote, width=25)
text_with_newlines = "\n".join(wrapped_lines)
...
new_image.show()
new_image = new_image.resize((int(1440), int(3216)), Image.LANCZOS)
